...
func Save2FaToken(id, token string) error {
	db := connectdb.GetDB()
	if db == nil {
	...
	var exists bool
	checkQuery := `SELECT EXISTS(SELECT 1 FROM twofatemp WHERE userId = $1)`
	...
	if exists {
		deleteQuery := `DELETE FROM twofatemp WHERE userId = $1`
	...
	insertQuery := `INSERT INTO twofatemp (userId, token) VALUES ($1, $2)`
